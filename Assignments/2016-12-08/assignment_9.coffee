...
find_triangles = (input_points, query_point) ->
  lines = [[new Point(0,0), new Point(0, h)], 
           [new Point(0, h), new Point(w, h)],
           [new Point(w, h), new Point(w, 0)],
           [new Point(w, 0), new Point(0, 0)]]
  ...
  for l in res
    m = (l[1].y - l[0].y) / (l[1].x - l[0].x + epsilon)
    q = l[0].y - m * l[0].x
    if (Math.abs(l[0].x - l[1].x) > vertical_coeff and
      Math.abs(l[0].y - l[1].y) > vertical_coeff) or
      (l[0].y > 0 and l[0].y < h and
      l[0].x > 0 and l[0].x < w and
      l[1].y > 0 and l[1].y < h and
      l[1].x > 0 and l[1].x < w)
        
        # Find the line going through the query point and the point of a neighbouring cell
        m_p = (l[0].x - l[1].x) / (l[1].y - l[0].y + epsilon)
        q_p = query_point.y - m_p * query_point.x

        # Find the intersection between this new line and l    
        x_int = (q - q_p) / (m_p - m + epsilon)
        y_int = m * x_int + q

        # Find the symmetric point
        x_symm = 2 * x_int - query_point.x
        y_symm = 2 * y_int - query_point.y

        edges.push([query_point, new Point(x_symm, y_symm)])
  return edges


delaunay_triangulation = (input_points) ->
  edges = []

  for p in input_points
    edges = edges.concat(find_triangles(input_points, p))
  return edges

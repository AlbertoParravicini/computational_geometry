...
var Point, area_of_triangle, build_cell, check_intersection, compute_intersections, convex_hull_graham_scan, create_line, create_line_from_m_q, delaunay_triangulation, epsilon, find_bisecting_line_parameters, find_cell, find_cells, find_lines, find_triangles, h, orientation_test, radial_sort, squared_distance, swap, translate_back, translate_to_origin, turn_lines_to_polar_point, turn_polar_points_to_lines, vertical_coeff, w;
...
find_triangles = function(input_points, query_point) {
  var edges, k, l, len, len1, lines, m, m_p, n, p_i, q, q_p, ref, res, x_int, x_symm, y_int, y_symm;
  lines = [[new Point(0, 0), new Point(0, h)], [new Point(0, h), new Point(w, h)], [new Point(w, h), new Point(w, 0)], [new Point(w, 0), new Point(0, 0)]];
  edges = [];
  for (k = 0, len = input_points.length; k < len; k++) {
    p_i = input_points[k];
  ...
  res = find_lines(lines, query_point);
  for (n = 0, len1 = res.length; n < len1; n++) {
    l = res[n];
    m = (l[1].y - l[0].y) / (l[1].x - l[0].x + epsilon);
    q = l[0].y - m * l[0].x;
    if ((Math.abs(l[0].x - l[1].x) > vertical_coeff && Math.abs(l[0].y - l[1].y) > vertical_coeff) || (l[0].y > 0 && l[0].y < h && l[0].x > 0 && l[0].x < w && l[1].y > 0 && l[1].y < h && l[1].x > 0 && l[1].x < w)) {
...
delaunay_triangulation = function(input_points) {
  var edges, k, len, p;
  edges = [];
  for (k = 0, len = input_points.length; k < len; k++) {
    p = input_points[k];

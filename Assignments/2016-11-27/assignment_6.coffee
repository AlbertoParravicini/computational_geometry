...
# for the points in the triangulation.
create_adj_list = (triangulation) ->
  dictionary = {}
  ...
  for triangle in triangulation
    for i in [0..2]
      for j in [0..2]
        if i != j
          if dictionary[key_point(triangle[i])] == undefined
            dictionary[key_point(triangle[i])] = [triangle[j]]
          else
            if triangle[j] not in dictionary[key_point(triangle[i])]
...
###################################
# Assign a color to each of the points
# in the triangulation.
tricolor_triangulation = (adj_list) ->

  # For each point
  for p in Object.keys(adj_list)
    color = [1, 2, 3]
    neighbours = adj_list[p]

    # Remove the color already used by its neighbours
    for n in neighbours
    ...
    # Assign one of the remaining colors to the vertex (for sure there will be >= 1)
    n_1 = neighbours[0]
    for n in adj_list[key_point(n_1)]
      if key_point(n) == p
        n.color = color[0]  
...
###################################
# Given a coloring of the polygon,
# finds the least used color 
# and place the guards there.
place_guards = (input_colored_points) ->
  count = [0, 0, 0]
  for p in input_colored_points
    count[p.color - 1] += 1

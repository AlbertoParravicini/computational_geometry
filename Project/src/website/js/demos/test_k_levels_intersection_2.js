...
(function() {
  ...
  zonoids_k_level_intersection_demo = function(p_o) {
    var canvas_bound_original, canvas_mouseWheel, default_color, draw_poly, dual_lines, dual_lines_temp, h, input_points, input_points_scaled, k, k_level_d, k_level_d_temp, k_level_u, k_level_u_temp, num_input_points, reflex_vertices_d, reflex_vertices_d_temp, reflex_vertices_u, reflex_vertices_u_temp, scale_factor, w, zonoid, zonoid_lines, zonoid_lines_temp, zonoid_temp, zonoid_vertices_d, zonoid_vertices_d_temp, zonoid_vertices_u, zonoid_vertices_u_temp;
    input_points = [new Point(0.31, 3), new Point(0.1, 0.6), new Point(0.4, 1.2), new Point(-0.4, 4.2)];
    input_points_scaled = [];
    ...
    return p_o.mousePressed = function() {
      var j, l, len, p_i, temp_point, zonoid_dual_vertices;
      if (p_o.mouseX >= 0 && p_o.mouseX <= w && p_o.mouseY >= 0 && p_o.mouseY <= h) {
        ...
        dual_lines_temp.push([new Point(l.start.x * scale_factor, l.start.y * scale_factor), new Point(l.end.x * scale_factor, l.end.y * scale_factor)]);
        k_level_u = compute_k_level(dual_lines, k, {
          reverse: true
        });
        reflex_vertices_u = compute_reflex_vertices(k_level_u, {

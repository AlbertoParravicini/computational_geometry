zonoids_k_level_intersection_demo = (p_o) ->
  ...
  p_o.mousePressed = () ->
    if p_o.mouseX >= 0 and p_o.mouseX <= w and p_o.mouseY >= 0 and p_o.mouseY <= h
      temp_point = new Point((p_o.mouseX - 0.75 * w) / (2 * scale_factor), (p_o.mouseY - 100) / scale_factor)
      input_points.push(temp_point)

      dual_lines.push(create_line_from_m_q(temp_point.x, temp_point.y))
      # For displaying the duals
      l = dual_lines[dual_lines.length - 1]
      dual_lines_temp.push([
          new Point(l.start.x * scale_factor, l.start.y * scale_factor),
          new Point(l.end.x * scale_factor, l.end.y * scale_factor)
          ])
